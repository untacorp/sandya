...
export interface SmpBeaconPacket {
  version: number;
  packetType: SmpPacketType.PRESENCE_BEACON | SmpPacketType.MESH_ANNOUNCE;
  sequence: number;
  timestamp: number;
  senderPeerId: string;
}
...
export const SMP_PACKET_CONSTANTS = {
  ...
  BEACON_SIZE: 16, // 1 + 1 + 2 + 4 + 8 = 16 Bytes (Fits in 31B Legacy BLE Adv)
  ...
  PAYLOAD_OFFSET: 28,
  BEACON_SEQ_OFFSET: 2,
  BEACON_TIMESTAMP_OFFSET: 4,
  BEACON_SENDER_OFFSET: 8,
} as const;
...
export class SmpPacketCodec {
  ...
  /**
   * Mengemas paket SMP v1/v2 ke dalam buffer biner bertanda tangan digital
   */
  ...
  /**
   * Mengemas Compact Presence Beacon 16 Byte (khusus Legacy BLE Advertising MTU 31B)
   */
  public static encodeBeacon(beacon: SmpBeaconPacket): Buffer {
    const buf = Buffer.alloc(SMP_PACKET_CONSTANTS.BEACON_SIZE);
    buf.writeUInt8(beacon.version || SMP_PACKET_CONSTANTS.CURRENT_VERSION, 0);
    buf.writeUInt8(beacon.packetType || SmpPacketType.PRESENCE_BEACON, 1);
    buf.writeUInt16BE(beacon.sequence, SMP_PACKET_CONSTANTS.BEACON_SEQ_OFFSET);
    buf.writeUInt32BE(
      beacon.timestamp || Math.floor(Date.now() / TIME_CONSTANTS.MS_PER_SECOND),
      SMP_PACKET_CONSTANTS.BEACON_TIMESTAMP_OFFSET
    );
    const senderBuf = Buffer.from(
      beacon.senderPeerId.padEnd(SMP_PACKET_CONSTANTS.PEER_ID_HEX_LENGTH, '0').slice(0, SMP_PACKET_CONSTANTS.PEER_ID_HEX_LENGTH),
      'hex'
    );
    senderBuf.copy(buf, SMP_PACKET_CONSTANTS.BEACON_SENDER_OFFSET, 0, SMP_PACKET_CONSTANTS.PEER_ID_BYTE_LENGTH);
    return buf;
  }

  /**
   * Mendekode Compact Presence Beacon 16 Byte
   */
  public static decodeBeacon(buffer: Buffer): Result<SmpBeaconPacket, DomainError> {
    if (buffer.length < SMP_PACKET_CONSTANTS.BEACON_SIZE) {
      return Err(
        new DomainError(
          'BEACON_TOO_SHORT',
          `Panjang beacon biner kurang dari ${SMP_PACKET_CONSTANTS.BEACON_SIZE}B.`,
    ...
    const packetType = buffer.readUInt8(1) as SmpPacketType.PRESENCE_BEACON | SmpPacketType.MESH_ANNOUNCE;
    const sequence = buffer.readUInt16BE(SMP_PACKET_CONSTANTS.BEACON_SEQ_OFFSET);
    const timestamp = buffer.readUInt32BE(SMP_PACKET_CONSTANTS.BEACON_TIMESTAMP_OFFSET);
    const senderPeerId = buffer
      .subarray(SMP_PACKET_CONSTANTS.BEACON_SENDER_OFFSET, SMP_PACKET_CONSTANTS.BEACON_SENDER_OFFSET + SMP_PACKET_CONSTANTS.PEER_ID_BYTE_LENGTH)
      .toString('hex');
  ...
  /**
   * Mendekode buffer biner paket SMP v1/v2 dan memverifikasi integritas tanda tangan
   */
