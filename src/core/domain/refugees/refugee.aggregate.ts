...
import { validateVulnerabilities, sanitizeVulnerabilities } from './vulnerability-rules';
...
export class RefugeeAggregate {
  private constructor(
    ...
    private vulnerabilities: VulnerabilityCategory[],
    private urgentNeeds: string[],
  ...
  public static create(props: CreateRefugeeProps): Result<RefugeeAggregate, DomainError> {
    ...
    const rawVulnerabilities = props.vulnerabilities || [];
    const valResult = validateVulnerabilities(props.gender, props.age, rawVulnerabilities);
    if (!valResult.ok) {
      return Err(valResult.error);
    }

    const cleanVulnerabilities = sanitizeVulnerabilities(props.gender, props.age, rawVulnerabilities);
    const cleanUrgentNeeds = (props.urgentNeeds || []).map((n) => String(n).trim()).filter(Boolean);

    const initialTriage: TriageCategory = cleanVulnerabilities.includes('LUKA_BERAT') ? 'RED' : 'GREEN';
    const now = Date.now();
  ...
  public updateVulnerabilities(vulnerabilities: VulnerabilityCategory[]): Result<void, DomainError> {
    const valResult = validateVulnerabilities(this.gender, this.age, vulnerabilities);
    if (!valResult.ok) {
      return Err(valResult.error);
    }
    this.vulnerabilities = sanitizeVulnerabilities(this.gender, this.age, vulnerabilities);
    this.version += 1;
    return Ok(undefined);
  }

  public updateUrgentNeeds(needs: string[]): void {
    this.urgentNeeds = needs.map((n) => String(n).trim()).filter(Boolean);
    this.version += 1;
  }
