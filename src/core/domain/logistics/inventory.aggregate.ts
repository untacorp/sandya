...
export class InventoryAggregate {
  ...
  public static create(props: CreateInventoryProps): Result<InventoryAggregate, DomainError> {
    if (!props.itemName.trim()) {
      return Err(new DomainError('INVALID_ITEM_NAME', 'Nama barang logistik tidak boleh kosong.', 422));
    }
    if (props.initialQuantity < 0) {
      return Err(new DomainError('INVALID_QUANTITY', 'Kuantitas awal stok tidak boleh negatif.', 422));
    }
  ...
  public mutateStock(
    ...
    if (quantityChange === 0) {
      return Err(new DomainError('ZERO_CHANGE', 'Perubahan kuantitas tidak boleh 0.', 422));
    }
