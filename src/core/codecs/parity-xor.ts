...
export class XorParityEngine {
  ...
  public static splitWithParity(
    ...
    const numChunks = Math.max(1, targetDataChunks);
  ...
  public static assembleFromChunks(
    ...
    if (dataMap.size === expectedDataChunks - 1 && parityChunk) {
      ...
      if (missingIndex !== -1) {
        const expectedLen = originalLengths[missingIndex] ?? parityChunk.data.length;
        ...
        dataMap.set(missingIndex, {
          index: missingIndex,
          totalChunks: expectedDataChunks + 1,
          isParity: false,
    ...
    throw new Error(
      `INSUFFICIENT_CHUNKS: Memerlukan minimal ${expectedDataChunks} dari ${expectedDataChunks + 1} chunks untuk rekonstruksi.`
    );
  }
}
