...
export default function OrganizationConsolePage() {
  ...
  const [syncToast, setSyncToast] = React.useState<{ message: string; isError?: boolean } | null>(null);

  const org = organizations.find((o) => o.id === session.orgId) || organizations[0] || {
    id: session.orgId || "ORG-01",
    name: session.orgName || "Pusat Komando Wilayah",
    category: "BPBD_PEMERINTAH" as const,
    masterPubkey: "did:sandya:org_local",
    createdAt: Date.now(),
  };
  ...
  const handleCloudSync = async () => {
    ...
    setSyncToast({ message: res.message, isError: !res.success });
    setTimeout(() => setSyncToast(null), 4500);
  };

  const getOrgCategoryLabel = (cat?: string) => {
    switch (cat) {
      case "PMI_LEMBAGA":
        return "Palang Merah Indonesia";
      case "BPBD_PEMERINTAH":
        return "BPBD / Pemerintah Daerah";
      case "NGO_YAYASAN":
        return "Yayasan / NGO Kemanusiaan";
  ...
  return (
    <div className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-surface-muted border border-border text-text-muted">
                {getOrgCategoryLabel(org.category)}
              </span>
            </div>
      ...
      {syncToast && (
        <div
          className={`p-3 rounded-xl text-xs font-semibold flex items-center gap-2 shadow-2xs border ${
            syncToast.isError
              ? "bg-status-danger-bg border-status-danger text-status-danger"
              : "bg-status-safe-bg border-status-safe-border text-status-safe"
          }`}
        >
          <Icon name={syncToast.isError ? "sos" : "check"} variant="bold" size={16} />
          <span>{syncToast.message}</span>
        </div>
      )}

      {/* 3. Tiga Ringkasan Utama (Spasi Rasional) */}
      ...
      {/* 4. Daftar Operasi Bencana yang Sedang Berjalan */}
