...
export default function OrgSettingsPage() {
  const { session, organizations, missions, poskos } = usePoskoStore();
  const org = organizations.find((o) => o.id === session.orgId) || organizations[0];

  const [orgName, setOrgName] = React.useState(org?.name || session.orgName || "PMI Kabupaten Cianjur");
  const [orgCategory, setOrgCategory] = React.useState(org?.category || "PMI_LEMBAGA");
  const [headquartersAddress, setHeadquartersAddress] = React.useState("Jl. Pangeran Hidayatullah No. 12, Cianjur");
  const [contactNumber, setContactNumber] = React.useState(org?.contactNumber || "+62 263 261 444");
  const [leadPersonName, setLeadPersonName] = React.useState(session.userName || "Budi Santoso");

  const [cloudType, setCloudType] = React.useState<"MANAGED" | "BYOC">("MANAGED");
  const [byocUrl, setByocUrl] = React.useState("https://sanidya.pmi-cianjur.org");
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-text-muted mt-0.5">
            Digunakan untuk sinkronisasi otomatis antar-perangkat saat posko mendapatkan akses internet.
          </p>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <p className="text-xs text-text-muted mt-1">
                Server pusat terkelola resmi siap pakai.
              </p>
            </button>

            <button
              type="button"
              onClick={() => setCloudType("BYOC")}
              className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
                cloudType === "BYOC"
                  ? "bg-primary/5 border-primary shadow-2xs"
                  : "bg-surface border-border hover:border-border-hover"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold text-text-main">
                  Server Instansi Sendiri
                </h4>
              </div>
              <p className="text-xs text-text-muted mt-1">
                Hubungkan dengan server data lokal instansi Anda.
              </p>
            </button>
          </div>

          {cloudType === "BYOC" && (
            <div className="space-y-2 p-3.5 rounded-xl bg-surface-subtle border border-border">
              <label className="text-xs font-semibold text-text-muted block">
                Alamat URL Server Instansi
              </label>
              <div className="flex flex-col sm:flex-row gap-2">
                <Input
                  value={byocUrl}
                  onChange={(e) => setByocUrl(e.target.value)}
                  placeholder="https://sanidya.pmi-cianjur.org"
                  className="flex-1 font-mono text-xs"
                />
                <Button variant="secondary" size="sm" onClick={() => alert("Koneksi server instansi aktif dan terverifikasi.")}>
                  Uji Sambungan
                </Button>
              </div>
            </div>
