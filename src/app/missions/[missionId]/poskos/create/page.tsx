...
export default function CreatePoskoPage() {
  ...
  const handleFinish = () => {
    if (createdPoskoId) {
      ...
      router.push(`/missions/${session.missionId}/poskos`);
    }
  };
  ...
  return (
    <div className="space-y-5 max-w-xl mx-auto">
      ...
      {/* Posko Lead Role Pass Modal */}
      <Dialog
        open={qrModalOpen}
        onOpenChange={setQrModalOpen}
        title="QR Masuk Koordinator Posko"
        description="Posko berhasil didaftarkan. Berikan QR ini kepada koordinator posko untuk dipindai."
        maxWidth="sm"
      >
        <div className="space-y-4 text-center">
          ...
          <div className="text-xs text-text-muted space-y-0.5">
            <p className="font-bold text-text-main text-sm">{coordinatorName}</p>
            <p>{name}</p>
          </div>
