# Analisis Kritis & Blueprint Arsitektur Sistem: Sanidya (Offline-First Disaster Management)

Dokumen ini merupakan evaluasi kritis awal terhadap ide sistem di [`docs/idea.md`](file:///home/auttomus/Documents/Code/PROJECT/sanidya_v2/docs/idea.md), berfokus pada kelayakan (*feasibility*), mitigasi *race condition*, penanganan limitasi QR code, serta blueprint arsitektur data terdistribusi.
...
> 📄 **[`docs/spesifikasi-mesh-dan-intercom.md`](file:///home/auttomus/Documents/Code/PROJECT/sanidya_v2/docs/spesifikasi-mesh-dan-intercom.md)** dan **[`docs/event-sourcing-dan-hierarki.md`](file:///home/auttomus/Documents/Code/PROJECT/sanidya_v2/docs/event-sourcing-dan-hierarki.md)**.
...
graph TD
    ...
    DB_A -.->|☁️ Tier 3: Saat Ada Internet / Starlink Uplink| Cloud[(Sanidya Central Cloud / BYOC)]
    DB_B -.->|☁️ Tier 3: Saat Ada Internet / Starlink Uplink| Cloud
```
...
2. **Pos B** memindai QR Pos A $\rightarrow$ Mengetahui Pos A punya 100 kg beras.
3. **Pos C** memindai QR Pos A $\rightarrow$ Mengetahui Pos A punya 100 kg beras.
...
1. **Prinsip *Single-Writer* per Stok Fisik:**
   * Pos lain **dilarang memotong atau mengklaim stok** milik pos lain secara sepihak dalam kondisi offline.
...
4. **Poster Multi-QR Paritas XOR (Moda Cetak Fisik):**
   * Data dicetak dalam bentuk Grid 4–8 QR monokrom dengan redundansi Paritas XOR sehingga kebal terhadap sobekan tiang tenda.
5. **Fallback Jalur Transfer Offline Non-Visual:**
   * Sediakan ekspor file terenkripsi `.sanidya` melalui **Wi-Fi Direct / Local Hotspot / Bluetooth Low Energy (BLE) / USB OTG**.
...
* Pengungsi bernama "Budi" pindah dari Pos A ke Pos B.
* Di Pos A tercatat belum menerima bantuan selimut; di Pos B didata ulang sebagai entitas baru dan meminta selimut lagi.
* Saat Pos A dan Pos B digabungkan, sistem dapat mengalami kebingungan apakah "Budi" adalah satu orang atau dua orang berbeda.
...
* Dalam kondisi darurat, oknum dapat membuat QR palsu untuk merekayasa lonjakan kebutuhan bantuan logistik.
...
| Dimensi | Desain Konvensional / Naif | Desain Tangguh (Sanidya v2) |
| :--- | :--- | :--- |
...
Dokumen ini menjadi acuan spesifikasi teknis untuk pengembangan modul data, sinkronisasi *offline-first*, komunikasi taktis lapangan, dan modul logistik di Sanidya.
