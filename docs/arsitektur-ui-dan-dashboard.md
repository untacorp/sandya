# Arsitektur Antarmuka (UI/UX) & Navigasi Dashboard: Sandya

> **Status**: Approved (Spesifikasi Antarmuka Resmi)  
> **Klasifikasi**: Desain Layout & Hirarki Navigasi  
> **Dokumen Terkait**: [Blueprint Frontend](./design/ui-ux/01-arsitektur-frontend-sandya.md) | [Pedoman Solar Icons](./design/ui-ux/02-pedoman-solar-icons.md) | [Token Warna Semantik](./design/ui-ux/03-sistem-desain-dan-token-warna.md)

Dokumen ini mendefinisikan rancangan antarmuka resmi sistem **Sandya (Offline-First Disaster Management)**, yang mencakup adaptasi tata letak responsif untuk dua moda operasional:
1. **Moda Mobile-First (Smartphone)**: Didesain untuk relawan lapangan dengan interaksi jempol (*thumb-friendly 5-Tab layout*) di kondisi darurat.
...
```text
┌─────────────────────────────────────────────────────────────┐
│  [Posko GOR Pacet]       8 Node (Mesh)        [Pengaturan]  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│                      KONTEN UTAMA LAYAR                     │
...
* **Hero KPI Cards**:
  * Total Pengungsi (misalnya `512 Jiwa` | `128 KK`).
...
* **Peringatan Stok Kritis (*Burn Rate Alert*)**:
  * *"Stok Susu Bayi diprediksi habis dalam 8 jam!"*
...
* **`[Distribusi]`**:
  * Penyerahan bantuan ke warga (Pindai warga -> serahkan barang -> tiket `COMPLETED`).
* **`[Bantuan Antar-Posko]`**:
...
* Membuka modal form **Fast Mobile Intake 30 Detik** dari layar mana pun tanpa perlu berpindah tab.
...
```text
┌──────────────┬──────────────────────────────────────────────────────────────┐
│  SANDYA      │ [Posko Induk Cianjur]  [Offline]  [12 Pending]  [User]       │
├──────────────┼──────────────────────────────┬───────────────────────────────┤
...
│  Pengaturan  │   Dusun Cijedil | Tenda 01   │ [Tambah Medis]  [Bantuan]     │
└──────────────┴──────────────────────────────┴───────────────────────────────┘
```
...
└── (posko)/[poskoId]/                          # Level 3: Dashboard Operasional Internal Posko
    ...
    ├── tactical/                               # 4. Komunikasi Taktis & Mesh Intercom
    ...
    └── settings/page.tsx                       # Pengaturan Kriptografi & Profil Posko Ini
```
