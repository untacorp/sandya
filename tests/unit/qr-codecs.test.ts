...
async function runQrTests() {
  console.log('🧪 RUNNING SANIDYA V2 QR CODECS TEST SUITE...\n');
  ...
  const sampleLargePayload = Buffer.from(
    'SANIDYA_OFFLINE_DISASTER_EVACUEE_DATA_'.repeat(100), // ~3.8 KB
    'utf8'
  );
  ...
  console.log('  ✅ Animated QR Out-of-Order Assembly Passed (100% Match)');
  ...
  console.log('  ✅ Parity Poster 1-QR Loss Recovery Passed (100% Recovery)');
  ...
  const verifyResult = RolePassCodec.verifyAndDecodePass(passQrString, masterKey.publicKeyHex);
  ...
  const forgedResult = RolePassCodec.verifyAndDecodePass(passQrString, fakeKey.publicKeyHex);
  assert.strictEqual(forgedResult.ok, false, 'Forged pass must be rejected');
  console.log('  ✅ Role Pass QR Cryptographic Verification Passed');
